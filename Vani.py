...
class Librarian:
    ...
    def return_book(self, book_id, student_id):
        ...
        if book and student and book_id in student.borrowed_books:
            ...
            # Get the issue_date from logs by matching book_id and student_id
            ...
            if issue_date:
                ...
                messagebox.showerror("Error", "Issue date not found in logs.")
                return None
        return None
    ...
    def calculate_penalty(self, issue_date):
        ...
        # Ensure the issue date is in the correct format ('%m/%d/%Y')
        try:
            issue_date = datetime.strptime(issue_date, '%m/%d/%Y')
        except ValueError:
            # If it doesn't match, try the '%Y-%m-%d' format
            issue_date = datetime.strptime(issue_date, '%Y-%m-%d')

        return_date = datetime.now()
        days_late = (return_date - issue_date).days - 2  # Assuming 2 days for free
        if days_late > 0:
            return min(days_late * 1, 50)  # 1 is the penalty per day, 50 is the max penalty
        return 0
