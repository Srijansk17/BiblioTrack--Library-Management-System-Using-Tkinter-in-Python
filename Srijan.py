...
class Librarian:
    ...
    def load_data(self):
        ...
        with open('students.csv', 'r') as file:
            reader = csv.reader(file)
            next(reader)  # Skip header
            self.students = [Student(row[0], row[1]) for row in reader]
    ...
    issue_date = datetime.strptime(issue_date, '%m/%d/%Y')  # Parse the date in MM/DD/YYYY format
    return_date = datetime.now()
    days_late = (return_date - issue_date).days - 2  # Assuming a 2-day grace period
    if days_late > 0:
        return min(days_late * 1, 50)  # Penalty is $1 per day, capped at $50
    return 0
